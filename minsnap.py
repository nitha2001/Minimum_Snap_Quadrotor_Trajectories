...
def add_continuity_constraints(prog, sigma, n, d, dt):
    for i in range(n-1):
        for k in range(1,5):
            expr = -factorial(k)*sigma[i+1][k]
            for j in range(k,d):
                expr += (factorial(j)/factorial(j-k))*sigma[i][j]*(dt[i]**(j-k))
            prog.AddLinearEqualityConstraint(expr, np.zeros(2))
...
def add_minsnap_cost(prog, sigma, n, d, dt):
  ...
  for i in range(n):
    for j in range(4,d):
        for k in range(4,d):
            cost += (((factorial(j) / factorial(j - 4))*(factorial(k) / factorial(k - 4))) * sigma[i, j].T@(sigma[i, k]) * (dt[i] ** (j + k - 7)))/(j+k-7)
